...
function M.lsp()
    ...
    keymap(n, "ge", function()
        require("peinan.native.lsp").open_diagnostics_float()
    end, default_settings)
    keymap(n, "[e", function()
        require("peinan.native.lsp").prev_diag()
    end, default_settings)
    keymap(n, "]e", function()
        require("peinan.native.lsp").next_diag()
    end, default_settings)
    keymap(n, "[E", function()
        require("peinan.native.lsp").prev_error()
    end, default_settings)
    keymap(n, "]E", function()
        require("peinan.native.lsp").next_error()
    end, default_settings)
...
function M.noice()
    local duration = require("peinan.plugins.neoscroll-nvim").scroll_duration
...
function M.native()
    ...
    keymap(n, "<leader>d", function()
        require("peinan.native.lsp").toggle_virtual_diagnostics()
    end, default_settings)
    keymap(n, "<leader>f", function()
        require("peinan.native.lsp").toggle_format_enabled()
    end, default_settings)
    keymap(n, "gm", "<Cmd>vertical Man<CR>", default_settings)

    -- Buffers
    keymap(n, "Q", function()
        require("peinan.keymaps.utils").delete_buffer()
    end, default_settings)
...
function M.editing()
    ...
    keymap(ex_t, "<C-s>", function()
        require("peinan.keymaps.utils").save_file()
    end, default_settings)
end
